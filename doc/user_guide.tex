...
set(SPDLOG_PATH $ENV{HOME}/builds/spdlog-install
    CACHE STRING "")
\end{verbatim}
\item An MPI compiler is optional but recommended in order to exploit multiple cores.
\item Link against a Scalapack implementation in addition to MPI to exploit multiple cores even more.
\end{itemize}
...
If the derivatives were calculated with finite differences, a simple usage of OpenMP would be sufficient to parallelize most of GADfit. However, with AD, both function evaluation and the gradient calculation have side effects, which means that a simple parallelization scheme is not possible. The C++ version of GADfit is parallelized using MPI. In particular, all computations that run over the number of data points (Jacobian, residuals, sum of squares, \ldots), are declared as shared arrays making use the MPI remote memory access (RMA) paradigm. If we stop here then the rest of the linear algebra is done in serial. However, if you have access to a Scalapack implementation, the linear algebra part can run in parallel as well. You may build the Netlib reference implementation of Scalapack yourself or use, e.g., the one supplied with MKL which, at the time of writing, is freely available.
...
\item \verb+blacs_single_column+: whether to create an $N\times1$ BLACS process grid where $N$ is the number of processes. This might be suitable if the number of data points is (much) higher than the number of fitting parameters. If false, then a rectangular process grid is created with \verb+MPI_Dims_create+.
\item \verb+min_n_blocks+: Minimum number of blocks of a 2D block-cyclic array held by each process. For minimal MPI communication set this values to 1 but for better load balancing set it to a larger value. Set \texttt{spdlog} level to \texttt{debug} in order to see details about the structure of block cyclic arrays.
\item \verb+prepare_getters+: Use this if you want to use getters after the fitting procedure has ended (e.g. \texttt{getJacobian()}).
\end{itemize}
...
\end{verbatim}
Get $\bm J$, $\bm J^T\bm J$, $\bm D^T\bm D$, left side of Eq.~\eqref{eq:LM}, right side of Eq.~\eqref{eq:LM}, or the residuals $\bm y - \bm f(\bm \beta)$, respectively. The setting \verb+prepare_getters+ must be set before the fitting procedure. Otherwise, these quantities are not returned correctly.
...
\end{itemize}
The function body may also refer to an external function. The dilogarithm function, $\Li2$, is an example of importing special functions from GSL. $\Li2$ is available if the CMake option \verb+USE_GSL+ is enabled (default) and CMake is able to automatically find the library (if not, try the \verb+GSL_ROOT_DIR+ variable). This is not required for running any of the tests.
