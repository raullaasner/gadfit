...
\item $\erf(x_1) = \frac{2}{\sqrt{\pi}} \int_0^{x_1} e^{-t^2} \D t$,
  \quad $\Litwo(x_1) = -\int_0^{x_1} \frac{\ln(1-u)}{u} \D u$,
% integrals
\item
...
./main
\end{verbatim}
With the multi-image version the number of images is set through the environment,
\begin{verbatim}
GFORTRAN_NUM_IMAGES=2 ./main
\end{verbatim}
(\verb+FOR_COARRAY_NUM_IMAGES+ with the Intel compiler).
...
# GNU
FC=gfortran
FFLAGS="-fcoarray=lib -J$GAD/include -L$GAD/lib"
LIBS="$LIBS -lcaf_shmem"
# Intel
#FC=ifort
...
GFORTRAN_NUM_IMAGES=4 ./main # GNU
#./main                      # Intel
\end{verbatim}
This may need to be modified to reflect the user's environment. The Coarray runtime library must be the same one that GADfit was built against (see Sec.~\ref{sec:parallelism_fortran}); with \verb+caf_single+ replace \verb+-lcaf_shmem+ by \verb+-lcaf_single+. If GADfit was built without \verb+-fcoarray=lib+, no Coarray runtime library need be linked.
...
In order to compile a Coarray program with GFortran, the compiler flag \texttt{-fcoarray=lib} must be used and a Coarray runtime library must be linked. GFortran ships two such libraries: \verb+libcaf_single+, which supports a single image only, and, since GCC 15, \verb+libcaf_shmem+, where the images are separate processes communicating through shared memory. Which one is used is determined by the \verb+CAF_LIBRARY+ configuration variable,
\begin{verbatim}
  cmake -D CAF_LIBRARY=caf_shmem ..
\end{verbatim}
If it is left unset, \verb+caf_single+ is used. The number of images is set with the \verb+GFORTRAN_NUM_IMAGES+ environment variable, which by default equals the number of cores,
\begin{verbatim}
  GFORTRAN_NUM_IMAGES=4 ./main
\end{verbatim}
The same applies to the tests, which are launched through the \verb+TESTS_RUN_PREFIX+ variable, e.g.\ \\
\verb+-D TESTS_RUN_PREFIX="env;GFORTRAN_NUM_IMAGES=4"+.
