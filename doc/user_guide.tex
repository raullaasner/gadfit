...
\end{verbatim}
If you look into \verb+lm_solver_data.h+ you'll see that \verb+x_data_1+ and \verb+y_data_1+ are of type \texttt{std::array<double>} and \verb+x_data_2+ and \verb+y_data_2+ are of type \verb+std::shared_ptr<std::vector<double>>+. This is just to demonstrate two different approaches. The latter is better because it avoids an unnecessary copy of data. But for convenience the container solution is also supported. Then set the initial values of all fitting parameters. In this example, all starting values are set to 1.0.
\begin{verbatim}
...
\end{verbatim}
Convenience function for adding a data set. This is an easy way to support containers of any kind but at the expense of a copy of all data.
\begin{itemize}
...
\begin{verbatim}
  cmake --build . --target test
\end{verbatim}
or you can them manually one by one. Each test is an executable in \texttt{c++/tests} in your build directory.
